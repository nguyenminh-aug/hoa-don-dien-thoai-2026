...
export function InvoiceItemEditor({ item, settings, index, isRemovable, onChange, onRemove }: InvoiceItemEditorProps) {
  ...
  return <tr>
    ...
    <td><input aria-label={`Mã hàng dòng ${index + 1}`} value={item.productName} onChange={event => onChange({ ...item, productName: event.target.value })} placeholder="Mã hàng" /></td>
    ...
    <td><select aria-label={`ĐV hàng ${index + 1}`} value={item.priceMode} onChange={event => onChange({ ...item, priceMode: event.target.value as InvoiceItemDraft['priceMode'] })}><option value="ndt">NDT</option><option value="vnd">VNĐ</option></select></td>
    ...
    <td><select aria-label={`Nguồn hàng ${index + 1}`} value={item.fromInventory ? 'inventory' : 'new'} onChange={event => changeSource(event.target.value === 'inventory')}><option value="new">Mới</option><option value="inventory">Tồn</option></select></td>

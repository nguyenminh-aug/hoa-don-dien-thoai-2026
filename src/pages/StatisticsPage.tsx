...
export function StatisticsPage() {
  ...
  const totals = useMemo(() => { const all = sourceTotals(allItems); const invoiceCosts = filtered.reduce((sum, invoice) => sum + (invoice.operatingCostTotal || 0), 0); const extraCosts = filteredExpenses.reduce((sum, expense) => sum + expense.total, 0); const manualDebt = debts.reduce((sum, debt) => sum + debt.remaining, 0); const invoiceProfit = (invoice: typeof filtered[number]) => invoice.profit ?? invoice.subtotal - (invoice.chinaCostTotal || 0) - (invoice.operatingCostTotal || 0) - (invoice.itemOperatingCostTotal || 0); const uncollectedProfit = filtered.reduce((sum, invoice) => sum + (invoice.subtotal > 0 ? invoiceProfit(invoice) * Math.min(1, Math.max(0, invoice.remaining / invoice.subtotal)) : 0), 0); const remaining = filtered.reduce((sum, invoice) => sum + invoice.remaining, 0) + manualDebt; const profit = filtered.reduce((sum, invoice) => sum + invoiceProfit(invoice), 0) - extraCosts; const actualProfit = profit - uncollectedProfit; const supplierPaid = supplierPayments.reduce((sum, payment) => sum + payment.amount, 0); return { ...all, operatingCosts: invoiceCosts + extraCosts, paid: filtered.reduce((sum, invoice) => sum + invoice.deposit + invoice.paid, 0), remaining, profit, uncollectedProfit, actualProfit, supplierPaid, availableCash: Math.max(0, actualProfit - supplierPaid) } }, [allItems, filtered, filteredExpenses, debts, supplierPayments])
  const metrics = [['Hóa đơn', String(filtered.length)], ['Khách hàng', String(customers.length)], ['Tổng số đôi', String(totals.quantity)], ['Doanh thu', formatVnd(totals.revenue)], ['Tiền gốc TQ', formatVnd(totals.chinaCost)], ['Tiền cửu', formatVnd(totals.tienCuu)], ['Chi phí vận hành', formatVnd(totals.operatingCosts)], ['Lợi nhuận', formatVnd(totals.profit)], ['Lợi nhuận thực tế', formatVnd(totals.actualProfit)], ['Đã trả NCC TQ', formatVnd(totals.supplierPaid)], ['Tiền mặt khả dụng', formatVnd(totals.availableCash)], ['Đã thu', formatVnd(totals.paid)], ['Còn nợ', formatVnd(totals.remaining)]]
